...
if discount_sales_file and discount_price_file and normal_sales_file:
    ...
    discount_grouped = discount_sales.groupby(["Product ID", "Hub ID"]).agg(
        qty_sold=("Qty sold Discounted Price", "sum"),
        discount_days=("Date", "nunique")
    ).reset_index()
    
    normal_grouped = normal_sales.groupby(["Product ID", "Hub ID"]).agg(
    ...
    df = df.merge(normal_grouped, on=["Product ID", "Hub ID"], how="left")
